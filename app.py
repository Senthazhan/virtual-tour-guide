...
def new_chat():
    ...
    if current_history:
        # Save to a separate session key for chat history preservation
        if "saved_chats" not in session:
            session["saved_chats"] = []
        
        # Add current chat to saved chats with timestamp
        from datetime import datetime
        chat_summary = {
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "first_message": current_history[0].get("text", "Chat") if current_history else "Chat",
            "message_count": len(current_history),
            "chat_id": str(uuid.uuid4())
        }
        session["saved_chats"].append(chat_summary)
    
    # Clear current conversation history for new chat
    session["history"] = []
